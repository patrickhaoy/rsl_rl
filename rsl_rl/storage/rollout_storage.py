...
class RolloutStorage:
    class Transition:
        def __init__(self) -> None:
            ...
            self.terminal_values: torch.Tensor | None = None
    ...
    def __init__(
        ...
        if training_type == "rl":
            ...
            self.terminal_values = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def add_transitions(self, transition: Transition) -> None:
        ...
        if self.training_type == "rl":
            ...
            self.terminal_values[self.step].copy_(transition.terminal_values)
    ...
    def compute_returns(
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            # For done steps, bootstrap with V(terminal_obs) instead of 0
            # For non-done steps, use V(s_{t+1}) as usual
            is_done = self.dones[step].float()
            next_value = (1.0 - is_done) * next_values + is_done * self.terminal_values[step]
            # TD error: r_t + gamma * V_next - V(s_t)
            delta = self.rewards[step] + gamma * next_value - self.values[step]
            # Advantage: A(s_t, a_t) = delta_t + gamma * lambda * (1 - done) * A(s_{t+1}, a_{t+1})
            advantage = delta + (1.0 - is_done) * gamma * lam * advantage
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        for epoch in range(num_epochs):
            # Reshuffle indices each epoch for better gradient diversity
